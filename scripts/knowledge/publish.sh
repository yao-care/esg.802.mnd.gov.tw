...
PROJECT_ROOT="$(cd "$PUBLISH_DIR/../.." && pwd)"
source "$PUBLISH_DIR/../lib/shell-config.sh"

MERGE="$PUBLISH_DIR/merge.sh"
...
if [ -d "$PROJECT_ROOT/$PROJECTS_PATH" ]; then
  ...
  for proj_dir in "$PROJECT_ROOT/$PROJECTS_PATH"*/; do
    ...
    "$PUBLISH_DIR/generate-project-dashboard.sh" "$proj_name" "$OUTPUT/projects/$proj_name" 2>/dev/null || echo "  WARN: dashboard for $proj_name failed"
  done
fi
...
# Generate index
"$PUBLISH_DIR/generate-dashboard.sh" "$OUTPUT" "$DOCS_DIR"
